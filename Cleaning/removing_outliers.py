...
def IQR_for_outliers(data, property_types, output_dir):
    ...
    for property_type in property_types:
        ...
        data_no_outlier = pro[
            (pro["price"] <= upper_limit) & (pro["price"] >= lower_limit)
        ]  # Remove outliers

        individual_filename = (
            f"{property_type}_without_outliers.csv"  # Saving the data separetly
        )
        data_no_outlier.to_csv(f"{output_dir}/{individual_filename}", index=False)

        combined_data_no_outlier = pd.concat(
            [combined_data_no_outlier, data_no_outlier], ignore_index=True
